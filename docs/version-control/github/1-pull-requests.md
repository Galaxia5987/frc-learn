...
After finishing working on a branch, we may want to merge it's changes to the `main` branch. After pushing our new changes (which can be made over a bunch of commits, it's a seperate branch and thus have a seperate history) to the branch, we can create a Pull Request.
